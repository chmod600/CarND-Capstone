#!/usr/bin/env python
# Use for reference - Richard used this to test the dbw nodes development
...
class WaypointUpdater(object):
    ...
    **Diagram (5) - New target trajectory planned - PLANNING**
    ...
    def cte_calc(self, pose, waypoints, car_index, POINTS_TO_FIT):
        ...
        x_coords, y_coords = self.transform_waypoints(pose, waypoints, car_index, POINTS_TO_FIT)
        coefficients = np.polyfit(x_coords, y_coords, 3)
        distance = np.poly1d(coefficients)

        return distance, x_coords, y_coords

    def transform_waypoints(self, pose, waypoints, car_index, points_to_use=None):

        x_coords = []  # transformed waypoint x
        y_coords = []  # transformed waypoint y
